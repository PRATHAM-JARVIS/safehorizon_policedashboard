...
const APITest = () => {
  ...
  const [authToken, setAuthToken] = useState(localStorage.getItem('safehorizon_auth_token'));
  ...
  const testLogin = async () => {
    ...
    try {
      const response = await authAPI.loginAuthority(loginForm.email, loginForm.password);
      localStorage.setItem('safehorizon_auth_token', response.access_token);
      setAuthToken(response.access_token);
      setLoginStatus('success');
      // Re-run API tests after successful login
  ...
  const testAPI = async () => {
    ...
    for (const test of testEndpoints) {
      try {
        ...
        // For endpoints that require auth, skip if no token
        if (test.requiresAuth && !localStorage.getItem('safehorizon_auth_token')) {
          results.push({
            ...test,
            status: 'skipped',
            error: 'No authentication token available. Please login first.',
            statusCode: 'Auth Required'
          });
          continue;
        }
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <Badge className={authToken ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
            ...
            {!authToken && (
              <div className="space-y-3">
                <p className="text-sm text-gray-600">Login with authority credentials to test protected endpoints:</p>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  ...
                  <Button 
                    onClick={testLogin} 
                    disabled={loginStatus === 'testing' || !loginForm.email || !loginForm.password}
                  >
                    {loginStatus === 'testing' ? 'Logging in...' : 'Test Login'}
                  </Button>
                </div>
                {loginStatus === 'error' && (
                  <p className="text-sm text-red-600">Login failed. Check credentials and backend connection.</p>
                )}
                {loginStatus === 'success' && (
                  <p className="text-sm text-green-600">Login successful! API tests will now include authenticated endpoints.</p>
                )}
              </div>
            )}
          </div>
