...
const Dashboard = () => {
      ...
      onMessage: (event) => {
        try {
          ...
          setStats((prev) => ({
            ...
            sosCount:
              alert.type === "sos" ? prev.sosCount + 1 : prev.sosCount,
          }));
        } catch (e) {
  ...
  // Handle incoming WebSocket messages (for debugging/logging)
  useEffect(() => {
    if (lastMessage) {
  ...
  // Color mapping for StatCard backgrounds
  const colorMap = {
  ...
  const StatCard = ({
    ...
    const bgColor = colorMap[color] || "bg-primary/10";
  ...
  const getSeverityColor = (severity) => {
    switch (severity) {
      ...
      case "high":
        return "destructive";
      case "medium":
        return "outline"; // fallback if "warning" not supported
      case "low":
        return "secondary";
      default:
        return "secondary";
