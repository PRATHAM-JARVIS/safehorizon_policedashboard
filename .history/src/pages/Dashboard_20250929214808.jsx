...
const Dashboard = () => {
  ...
  // WebSocket for real-time alerts (optional, won't block rendering)
  let wsConnection = null;
  try {
    const { lastMessage, readyState } = useWebSocket(
      `${import.meta.env.VITE_WS_BASE_URL || 'ws://localhost:8000/ws'}/alerts/subscribe`,
      {
        shouldReconnect: () => false, // Don't auto-reconnect if backend is not available
        onMessage: (alert) => {
    ...
    wsConnection = { lastMessage, readyState };
  } catch (error) {
    console.warn('WebSocket not available:', error);
    wsConnection = { lastMessage: null, readyState: 3 }; // CLOSED state
  }

  // Handle incoming WebSocket messages
  useEffect(() => {
    if (wsConnection?.lastMessage) {
      console.log('WebSocket message received:', wsConnection.lastMessage);
    }
  }, [wsConnection?.lastMessage]);
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        try {
          ...
          console.warn('Failed to fetch tourists:', apiError);
        ...
        try {
          alerts = await alertsAPI.getRecentAlerts({ limit: 10 });
        } catch (apiError) {
          console.warn('Failed to fetch alerts:', apiError);
          // Use mock data if API fails
          alerts = [
            {
              id: 1,
        ...
        // Set mock alerts for offline mode
        setRecentAlerts([
          {
            id: 1,
            type: 'geofence',
            severity: 'medium',
            title: 'Demo Alert - Geofence Violation',
            tourist_id: 'DEMO001',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {error && (
            <div className="flex items-center space-x-2 px-3 py-1 rounded-full text-sm bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-100">
          ...
          <div className={`flex items-center space-x-2 px-3 py-1 rounded-full text-sm ${
            ...
            <div className={`w-2 h-2 rounded-full ${wsConnection?.readyState === 1 ? 'bg-green-500' : 'bg-red-500'}`}></div>
            <span>{wsConnection?.readyState === 1 ? 'Live Updates' : 'Disconnected'}</span>
          </div>
        </div>
      </div>
